...
- `structuredContext` 是门店实时经营快照（每次对话由系统查库生成），区块与含义：
  ...
  这六个区块就是当天的全量事实：除此之外没有更多经营数据，你的所有取舍都在这些事实之内做。
