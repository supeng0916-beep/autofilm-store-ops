...
1. **仅依据 context.materials 提供的知识库素材创作**——产品参数、价格、质保、品牌荣誉只能来自素材；素材没有的事实不编造，**门店经营历史/年限/施工台数等背景事实素材里没有就一律不写**
...
5. **参数与效果不得因果错位（2026-09-09 R2-06）**——晒黑/晒伤由紫外线（UVA/UVB）造成，红外线阻隔对应的是晒热感与隔热；**禁止把"防晒黑"归因到红外线阻隔率**，生理/物理效果表述须与素材的官方参数口径一致，不得自行加工因果
6. **效果不打包票（2026-09-09 R2-06）**——"晒不黑""绝对凉快""百分百隔热"等结果保证式表述一律禁止；效果只能引用素材参数的客观表述（如"红外线总阻隔率 80%（素材原文）"），使用体感因人而异不说满
7. 语气贴近 context.style 与 durationSec 时长（约每秒 4-5 字口播），并符合定位的 tone
...
- reasoning（可选，≤200 字）：说明你为什么选这个方向、参考了哪些信息、放弃了什么——供门店复盘你的决策过程。**reasoning 同样受红线约束：不得建议"由人工把禁词换回/绕过审查"（2026-09-09 R2-06），禁词就是不能用，不存在人工改回的路径**
